...
# /etc/puppetlabs/code/environments/staging/site/roles/manifests/backend.pp

# @summary Role: RHEL backend (database tier) node for the staging environment.
#
# A role defines WHAT a node IS by assembling profiles.
# This role is assigned to all staging backend nodes.
#
# Profiles included:
...
#
# Node naming convention (see manifests/site.pp): stg-backend-<name>.example.com
#
class roles::backend {
