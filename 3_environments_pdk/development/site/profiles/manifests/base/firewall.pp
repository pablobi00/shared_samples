...
# /etc/puppetlabs/code/environments/development/site/profiles/manifests/base/firewall.pp
...
class profiles::base::firewall {
  ...
  # Add SSH as a permanently allowed service in the default firewall zone.
  # 'unless' checks if SSH is already allowed before running the command.
  # '--permanent' writes the rule to disk so it survives a reload or reboot.
  ...
  # Reload firewalld to apply any permanent rule changes made above.
  # 'refreshonly => true' means this exec ONLY runs when notified by another resource.
